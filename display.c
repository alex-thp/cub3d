...
/*   Updated: 2019/12/14 09:07:45 by thverney         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	display_ray(t_map *tab)
{
	...
	{
		tab->len.textY = (((double)i * 2. - (double)tab->res_y + (double)tab->len.heightline) * (64 / 2) / (double)tab->len.heightline);
		if (tab->len.side == 1 && tab->map_y < tab->pos_y)
		{
			tab->mlx.pix[i * tab->res_x + tab->len.x] = tab->dst_N.pix[(int)tab->len.textY * 64  + tab->len.textX]; //0xff000000;
		}
